...
export default function HistoryPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<HistoryTreeNode[]> => {
      const data = await fetchJson<{ terms: HistoryTreeNode[] }>("/api/terms");
      return data.terms;
    },
  });
  ...
  const renameMutation = useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) => patchJson(`/api/terms/${id}`, { name }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["terms"] }),
  });

  const createFolderMutation = useMutation({
    mutationFn: ({ name, parentId }: { name: string; parentId: string | null }) =>
      postJson("/api/terms/folders", { name, parentId }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["terms"] }),
  });

  const moveMutation = useMutation({
    mutationFn: ({ id, parentId }: { id: string; parentId: string | null }) =>
      patchJson(`/api/terms/${id}`, { parentId }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["terms"] }),
  });

  const reorderMutation = useMutation({
    mutationFn: (orderedIds: string[]) => postJson("/api/terms/reorder", { orderedIds }),
