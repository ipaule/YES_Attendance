...
async function main() {
  ...
  // Create Sunday dates (global): March-April 2026
  const sundays: Date[] = [];
  ...
  for (const group of groups) {
    const teamCount = 5 + Math.floor(Math.random() * 2); // 5-6 teams
    ...
    for (let t = 0; t < teamCount; t++) {
      ...
      // Add date columns (사랑, 소망, 믿음 get global dates; 샬롬 gets its own)
      const teamDates = [];
      const datesToUse = group.name === "샬롬" ? sundays.slice(0, 10) : sundays;

      for (let d = 0; d < datesToUse.length; d++) {
        const s = datesToUse[d];
